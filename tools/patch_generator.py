...
class PatchGenerator:
	...
	def __init__(self, reference_rom: str, built_rom: str, patches_dir: str = "patches", timestamp: Optional[str] = None):
	...
	def generate_ips_patch(self) -> Optional[Path]:
		...
		try:
			...
			# Write IPS file with timestamp if provided
			if self.timestamp:
				ips_file = self.patches_dir / f"dragon_warrior_{self.timestamp}.ips"
			else:
	...
	def generate_bps_patch(self) -> Optional[Path]:
		...
		try:
			...
			# Write BPS file with timestamp if provided
			if self.timestamp:
				bps_file = self.patches_dir / f"dragon_warrior_{self.timestamp}.bps"
			else:
...
@click.option('--timestamp', help='Timestamp for patch filenames (YYYYMMDD_HHMMSS format)')
def generate_patches(reference_rom: str, built_rom: str, patches_dir: str, timestamp: Optional[str]):
	"""Generate IPS and BPS patches for ROM differences"""

	generator = PatchGenerator(reference_rom, built_rom, patches_dir, timestamp)
