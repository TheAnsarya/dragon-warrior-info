# Dragon Warrior Info Project - Documentation Index

Welcome to the comprehensive documentation for the Dragon Warrior NES disassembly and ROM hacking project.

## 📚 Documentation Structure

### 🚀 User Guides
Essential guides for getting started with the project.

- [**Quick Start Guide**](guides/QUICK_START.md) - Get up and running in 5 minutes
- [**Build Guide**](guides/BUILD_GUIDE.md) - Complete build system documentation
- [**Asset Extraction Guide**](guides/ASSET_EXTRACTION.md) - Extract graphics, text, and data
- [**Modding Quick Reference**](guides/MODDING_QUICK_REF.md) - Fast reference for common modding tasks
- [**FAQ**](guides/FAQ.md) - Frequently asked questions

### 🔧 Technical Documentation
In-depth technical information about Dragon Warrior's systems.

- [**ROM Structure**](technical/ROM_STRUCTURE.md) - Complete ROM layout and memory mapping
- [**SRAM Format**](technical/SRAM_FORMAT.md) - Save game data structure
- [**Graphics Format**](technical/GRAPHICS_FORMAT.md) - NES CHR-ROM and tile formats
- [**Text Encoding**](technical/TEXT_ENCODING.md) - Character encoding and compression
- [**Music Format**](technical/MUSIC_FORMAT.md) - Audio data and sound engine
- [**Game Mechanics**](technical/GAME_MECHANICS.md) - Battle system, stats, formulas

### 🏛️ Data Crystal Wiki
DataCrystal.org compatible wiki documentation for ROM hacking reference.

- [**ROM Map**](datacrystal/ROM_MAP.md) - Complete memory map in DataCrystal format
- [**RAM Map**](datacrystal/RAM_MAP.md) - Runtime memory usage
- [**SRAM Map**](datacrystal/SRAM_MAP.md) - Save data structure
- [**Data Structures**](datacrystal/DATA_STRUCTURES.md) - All game data tables
- [**Hacking Information**](datacrystal/HACKING_INFO.md) - Modification techniques and tools

### 🛠️ Development
Information for contributors and developers.

- [**Developer Onboarding**](guides/DEVELOPER_ONBOARDING.md) - How to contribute to the project
- [**Tool Development**](technical/TOOL_DEVELOPMENT.md) - Creating new analysis tools
- [**Testing Guide**](guides/TESTING_GUIDE.md) - Running and writing tests
- [**Architecture**](technical/ARCHITECTURE.md) - Project structure and design decisions

## 📊 Project Overview

This project aims to create a complete disassembly and modding environment for Dragon Warrior (NES), following the proven patterns established by the [FFMQ project](https://github.com/TheAnsarya/ffmq-info).

### 🎯 Goals
- **Complete ROM Documentation** - Every byte mapped and understood
- **Asset Extraction Tools** - Extract and edit all game content
- **Build Pipeline** - Reassemble modified ROMs
- **Visual Editors** - GUI tools for easy content modification
- **DataCrystal Wiki** - Comprehensive hacking reference

### ✅ Current Status
- 🏗️ **Foundation** - Project structure and build system
- 🐍 **Python Environment** - Virtual environment with dependencies  
- 🔧 **GitHub Integration** - Issues and project management automation
- 📝 **Documentation Framework** - Structured docs following FFMQ patterns
- 🎨 **Asset Extraction** - Basic ROM analysis and extraction tools

### 🔄 In Progress
- 📊 **ROM Analysis** - Detailed structure mapping
- 🎮 **Game Data Tools** - Character, item, and monster editors
- 🎨 **Graphics Tools** - CHR-ROM extraction and editing
- 📚 **Wiki Documentation** - DataCrystal format documentation

### ⏳ Planned
- 🎵 **Music Tools** - Audio extraction and editing
- 🧪 **Testing Framework** - Automated validation and testing
- 🔀 **Advanced Modding** - Complex ROM modifications
- 📦 **Distribution** - Package management and releases

## 🚀 Quick Links

### For New Users
1. [Quick Start Guide](guides/QUICK_START.md) - Start here!
2. [Build Guide](guides/BUILD_GUIDE.md) - Build your first ROM
3. [Asset Extraction](guides/ASSET_EXTRACTION.md) - Extract game content

### For Modders  
1. [Modding Quick Reference](guides/MODDING_QUICK_REF.md) - Fast reference
2. [ROM Structure](technical/ROM_STRUCTURE.md) - Understand the layout
3. [Data Structures](datacrystal/DATA_STRUCTURES.md) - Game data reference

### For Developers
1. [Developer Onboarding](guides/DEVELOPER_ONBOARDING.md) - Get started contributing
2. [Architecture](technical/ARCHITECTURE.md) - Project design
3. [Tool Development](technical/TOOL_DEVELOPMENT.md) - Creating new tools

## 🏷️ Document Standards

All documentation follows these standards:
- **Markdown Format** - GitHub-flavored markdown with extensions
- **CRLF Line Endings** - Windows standard for consistency
- **UTF-8 Encoding** - Unicode support for special characters
- **Linked Navigation** - Cross-referenced for easy browsing
- **Version Control** - Tracked changes with git history

## 🤝 Contributing

See [Developer Onboarding](guides/DEVELOPER_ONBOARDING.md) for information on contributing to this documentation.

## 📞 Support

- **GitHub Issues** - [Project Issues](https://github.com/TheAnsarya/dragon-warrior-info/issues)
- **GitHub Project** - [Project Board](https://github.com/users/TheAnsarya/projects/5)
- **Session Logs** - Check `~docs/session-logs/` for development history

---
**Last Updated:** November 18, 2025  
**Project Version:** 0.1.0 (Foundation)  
**Documentation Version:** 1.0